...
computeAllLattices <- function(j, Sig, all_nodes, root_S = NA, print.width=NA, VERB=1, max.iter=1e4) {
  if (any(is.na(root_S))) {
    root_S = all_nodes[-j]
  }
  S_candidates <- list()
...
# 8/3/2018
is.lattice.maximal <- function(lat, j, all_nodes) {
  maximal <- T
  curr_m <- lat$m
  R <- setdiff(all_nodes[-j], lat$M)
  if (length(R) > 0) {
    for (k in 1:length(R)){
      cands <- combn(R,k)
      for (i in 1:dim(cands)[2]) {
        cand_set <-  cands[,i]
        cand_lat <- computeLattice(j, union(curr_m, cand_set), Sig, all_nodes, should_sort=F)
        if ( is.subset(curr_m, cand_lat$m) ) {
          maximal <- F
          break
        }
      }
  ...
  if (!maximal) {lat <- cand_lat}

  return( list(maximal=maximal, lat=lat) )

}

QX <- function(lat,X) {
  if ( !is.subset(X,lat$M) ) {
    return(0)
  }
  spread <- setdiff(lat$M,lat$m)
  2^length(setdiff(spread,X))
}
QX_list <- function(lats, X) {
  sum( sapply(lats, function(lat) QX(lat,X)) )
}


find_uncovered_set <- function(lats, nodes) {
  # nodes is the set of elements to choose from
  n <- length(nodes)
  X <- c()
  sum_Q <- QX_list(lats,X)
  if (sum_Q == 2^n) {
    return(X)
  }

  for (k in 1:n){
    for (x in setdiff(nodes,X)) {
      sum_Q <- QX_list(lats,c(X,x))
      if (sum_Q < 2^(n-k)) {
        X <- c(X,x)
        break
      }
    ...
    if (sum_Q == 0) break
    if ((sum_Q == 1) && (length(X) == n-1)) {
      if (QX_list(lats, nodes) == 0) { # ground set is not covered
        X <- nodes
      } # else, ground set is covered, hence X itself is not covered; do nothing
      break
    }
  }
...
computeFullLatticeDecomp <- function(j, Sig, all_nodes, should_sort=F) {
  n <- length(all_nodes[-j])
  lats <- list()
  lats[[1]] <- computeLattice(j, all_nodes[-j], Sig, all_nodes, should_sort=should_sort)
  ...
  while (TRUE) {
    ...
    if (length(S) == 0) break
    K <- K+1
    lats[[K]] <- computeLattice(j, S, Sig, all_nodes, should_sort=should_sort)
